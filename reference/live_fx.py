...
W=Path(r'C:\Users\igfi\Documents\Codex\2026-09-08\i\work')
...
def make_effect(file,effectname,hp,col,control,flash=False,sample=2):
 ...
 for pair in effect['Pairs']:
  ...
  if an['Name']=='FLBeamAppearance':
   ...
   count=max(1,min(180,math.ceil(frequency*life)))
...
for hp in ['HpEngine01','HpEngine02']:
 make_effect('gf_li_smallengine02_fire','gf_li_smallengine02_fire',hp,enginecol,'engine_on')
 make_effect('gf_li_smallengine02_trail','gf_li_smallengine02_trail',hp,enginecol,'engine_on')
hp=next(o.name for o in bpy.data.objects if o.name.startswith('Thruster_HpThruster01::HpThrust'))
make_effect('gf_ge_s_thruster_01','gf_ge_s_thruster_01',hp,thrustcol,'thruster_on',sample=5)
for hp in ['HpContrail01','HpContrail02']:make_effect('li_contrail','li_contrail',hp,collection('Contrails'),'contrails_on')
for o in list(bpy.data.objects):
 if '::HpFire01' in o.name and (o.name.startswith('Gun_') or o.name.startswith('Turret_')):
  laser='li_laser_03' if o.name.startswith('Gun_') else 'li_laser_01'
  make_effect(laser,laser+'_flash',o.name,weaponcol,'weapon_effects',flash=True)
# Original light textures, values and individual phases; no invented illumination wattage.
lightcol=collection('Lights')
for hp,kind in [('HpHeadlight','head')]+[(f'HpRunningLight{i:02d}','blue') for i in range(1,5)]+[(f'HpDocklight{i:02d}','dock') for i in range(1,3)]:
 color,glow,bulb,gs=((155/255,)*3,(1,1,1),.7,4) if kind=='head' else (((100/255,100/255,1),)*2+(.1,.75)) if kind=='blue' else (((1,64/255,64/255),)*2+(2,3))
 for tex,size,colr in [('bulb',bulb,color),('shine',gs,glow)]:
  control='docking_lights' if kind=='dock' else 'running_lights';m=material('Light::'+hp+'::'+tex,tex,[colr]*2,[1,1],control)
  m['accuracy']='Recreated from light_equip.ini; additive original texture; flare-cone behavior approximated'
  # Tracking to presentation camera is added after the camera exists.
  o=meshobject('Light::'+hp+'::'+tex,[(-size,-size,0),(size,-size,0),(size,size,0),(-size,size,0)],[(0,1,2,3)],lightcol,bpy.data.objects[hp]);uv_default(o.data);o.data.materials.append(m)
  if kind in ['blue','dock']:
   g=next(n for n in m.node_tree.nodes if n.type=='VALUE');fc=g.outputs[0].driver_add('default_value');phase=int(hp[-2:])*13;fc.driver.expression=f'v*(1 if (frame+{phase})%{120 if kind=="blue" else 24}<{72 if kind=="blue" else 12} else {0.25 if kind=="blue" else 0.5})'
scene['Freelancer_FX_objects']=json.dumps(fxrecords)
bpy.context.view_layer.update()
result={'effect_systems':len(fxrecords),'lights':len(lightcol.objects),'packed_fx_images':len([i for i in bpy.data.images if i.name.startswith('FreelancerFX::')]),'saved':False}
